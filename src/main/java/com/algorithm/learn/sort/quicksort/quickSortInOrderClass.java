 ...
 */
public class quickSortInOrderClass {

    /**
     * 总结：
     * 1.改进快速排序时间：10000个数字排序，array 排序耗时：7ms
     */
...
//        int v=array[left]; //作为标准的元素，左侧第一个
        ...
        //随机取出一个[left,right]里面的值，使之和第一个数做交换。目的：使随机选出的元素作为标定
        int temp2=array[(int)Math.random()*(right-left+1)+left];
        array[(int)Math.random()*(right-left+1)+left]=array[left];
        array[left]=temp2;
        int v=array[left];
...
//        int[] arr=new int[]{1,50,81,64,3};
        ...
        new quickSortInOrderClass().quickSort(arr,0,arr.length-1); //快速排序
